...
round(duomenu_vidurkiai, digits = 2)
```
...
# Grafikai
## Grafikas sugeneruotas ggplot2
```{r pav-1}
grafikas <- ggplot(wdbc, aes(x = Diagnosis, y = Radius_mean, color = Texture_mean, size = Perimeter_mean)) + 
  geom_point(alpha = 0.5) +
  labs(
    x = "Diagnozė",
    y = "Vidurkis vežio skermens",
    title = "Gėrybinis vs piktybinį vėžį pagal parametrus",
    caption = "Grafikas vaiduojantis vėžio diagnozę pagal vėžio skermens, tekstūros ir perimetro vidurkius.  ") +
  theme_bw()
 
```
```{r pav-2, fig.cap=CAPTION}
CAPTION = "Grafikas"
grafikas
```
Atvaizduotas __grafikas__ apibūdinantis vėžį piktybiškumą pagal parametrus \@ref(fig:pav-2)

# Duomenų analizė
## Daugiamačių skalių (klasikinį PCoA)
```{r}
wd2 <- wdbc %>% select_if(is.numeric)
glimpse(wd2)
```
```{r}
wd2 %>% cor() %>% round(digits = 2)
```
### PCA sudarymas
```{r}
wd_pca <- prcomp(wd2, center = TRUE, scale = TRUE)
wd_pca
```
```{r}
get_eigenvalue(wd_pca) %>%
  round(1)
```
```{r}
fviz_screeplot(wd_pca, addlabels = TRUE, choice = "variance")
```

PCA bus sudaryta iš 5 dimensijų, kadangi deviacija paaiškina virš *80%* 5 dimensijos. 
## Hierarchinė duomenų analizė
### Duomenų standartizavimas
```{r}
wd3 <- wdbc %>% select_if(is.numeric)
wd5 <- scale(wd3)
```
### Trūkstamų reikšmių pašalinimas, jei tokių yra
```{r}
wd4 <- na.omit(wd5)
```
### Atstūmų skaičiavimas
```{r}
wd6 <- dist(wd4)
```
### Atstumų atvaizdavimas grafiku
```{r}
fviz_dist(wd6)
```
